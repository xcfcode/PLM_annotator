...
class BeamSearch(DecodeStrategy):
    ...
    def advance(self, log_probs, attn):
        ...
        if self.block_ngram_repeat > 0 and len(self) > 1:
            # assert self.block_ngram_repeat == 3
            # for each beam and batch sentence, generate a list of previous ngrams
            gen_ngrams = [{} for _ in range(self.beam_size)]
            for beam_id in range(self.beam_size):
                gen_tokens = self.alive_seq[beam_id].tolist()
                # print("gen_tokens", gen_tokens)
                for ngram in zip(*[gen_tokens[i:] for i in range(self.block_ngram_repeat)]):
                    # if tuple(ngram[:-1])[0] != self.end_t and tuple(ngram[:-1])[1] != self.start_t:
                    gen_ngrams[beam_id][tuple(ngram[:-1])] = \
                            gen_ngrams[beam_id].get(tuple(ngram[:-1]), []) + [ngram[-1]]

            def calculate_banned_tokens(beam_id):
                # before decoding the next token, prevent decoding of ngrams that have already appeared
                ngram_index = tuple(
                    # self.tokens[beam_id, cur_len + 2 - self.args.no_repeat_ngram_size:cur_len + 1].tolist())
                    # 最开始包含了<sos>
                    self.alive_seq[beam_id, len(self) + 1 - self.block_ngram_repeat:len(self)].tolist())
                return gen_ngrams[beam_id].get(ngram_index, [])

            if len(self) + 1 - self.block_ngram_repeat > 0:
                # no banned tokens if we haven't generated no_repeat_ngram_size tokens yet
                banned_tokens = [calculate_banned_tokens(beam_id) for beam_id in range(self.beam_size)]
            else:
                banned_tokens = [[] for _ in range(self.beam_size)]

            for beam_id in range(self.beam_size):
                log_probs[beam_id, banned_tokens[beam_id]] = -math.inf
